...
> **Vendored from [jiji262/claude-design-skill](https://github.com/jiji262/claude-design-skill)
> (MIT License, see `LICENSE` in this folder) on 2026-09-01.** Use this for
> one-off design artifacts (mockups, decks, posters, prototypes) — for
> ongoing work on the actual Richmond/NEXUS sites, use `web-dev-agent`
> instead, which already has this skill's anti-slop craft rules adapted
> for our static-site stack. The source repo's `assets/`/`demos/` starter
> templates and screenshots were not vendored — only `SKILL.md` and the
> `references/` docs — so anything pointing at `assets/` won't resolve here.
