...
> (MIT License, see `LICENSE` in this folder) on 2026-09-01, `ai-seo` skill
> only, v2.4.0 per its frontmatter.** Complements `seo-toolkit`'s
> `ai-visibility` entry with much deeper tactics (Princeton GEO
> citation-boost research, AI-bot robots.txt directives, llms.txt/
> pricing.md/OKF file specs, platform-by-platform behavior). The "Related
> Skills" and "Tool Integrations" sections below reference sibling skills
> (`seo-audit`, `schema`, `content-strategy`, `competitors`,
> `programmatic-seo`, `copywriting`, and a `tools/REGISTRY.md`) from the
> source repo that were **not** vendored here — for those, use the
> installed `searchfit-seo` plugin skills instead (see `seo-toolkit`).
